  ...
  return jsonResponse(
    {
      ...
      error: "We could not generate the mockup. Please try again or contact Chili."
    },
    500
  );
}
...
export default async function handler(request: Request) {
    ...
    return jsonResponse(
      {
        ...
        error: "Use POST to generate a mockup."
      },
      405
    );
  }
  ...
  try {
    ...
    const savedLogo = validateLogoFromJson(body);
    ...
    if (result.provider !== "gemini" || result.stubMode) {
      throw new AppError(
        "REAL_AI_REQUIRED",
        "Real Nano Banana Pro generation is required. Stub output is not allowed for generated mockups.",
        500
      );
    }
